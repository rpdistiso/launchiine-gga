...
BOOL StringTools::EndsWith(const std::string & a,
  const std::string & b) {
  if (b.size() > a.size())
    return false;
...
int32_t StringTools::strprintf(std::string & str,
  const char * format, ...) {
  static char tmp[512];
  tmp[0] = 0;
  int32_t result = 0;
...
int32_t StringTools::strtokcmp(const char * string,
  const char * compare,
...
int32_t StringTools::strextcmp(const char * string,
  const char * extension, char seperator) {
  if (!string || !extension)
    return -1;
...
std::vector < std::string > StringTools::stringSplit(const std::string & inValue,
...
bool StringTools::findStringIC(const std::string & strHaystack,
